...
- **A present-but-empty `?branch=` is now refused with `400` instead of serving an empty branch.** Both route layers defaulted the branch with `?? 'main'`, which only substitutes when the key is *absent* — under uws a present-but-empty `branch=` yields `''`, so the request created and served a room whose branch was the empty string, with its own redis stream and its own persisted snapshots. From the user's side the document silently "lost" content depending on which URL variant a client built. The websocket upgrade now denies with `400 invalid branch` and the rest endpoints answer `400` with `{ code: 'invalid-branch' }` (before authentication runs); an absent `?branch` still means `main`. **A client that sends `branch=` today starts getting `400`s** — send `branch=main` or omit the parameter. Anything it already wrote sits on the `''` branch and is *not* migrated; read it back with `yhub.getDoc({ org, docid, branch: '' }, { gc: true })` before upgrading if it matters. (yjs/yhub#68)
